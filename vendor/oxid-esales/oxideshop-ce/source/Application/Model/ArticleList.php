...
class ArticleList extends \OxidEsales\Eshop\Core\Model\ListModel
{
    ...
    public function loadCategoryArticlesNew($iLimit = 12)
    {

        $sArticleTable = getViewName('oxarticles');
        $sobject2category = getViewName('oxobject2category');
        $soxcategories = getViewName('oxcategories');
        $categoryName = 'Behandlungsliegen SALE';

        $sSelect = "select $sArticleTable.* from $sArticleTable inner join $sobject2category on $sobject2category.oxobjectid=$sArticleTable.oxid ";

        $sSelect .= "inner join $soxcategories on $sobject2category.oxcatnid=$soxcategories.oxid ";

        $sSelect .= "where $soxcategories.oxtitle like '$categoryName' and " .$this->getBaseObject()->getSqlActiveSnippet();
        $sSelect .= " limit " . $iLimit;
