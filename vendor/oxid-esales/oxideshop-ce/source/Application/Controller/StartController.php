...
class StartController extends \OxidEsales\Eshop\Application\Controller\FrontendController
{
     ...
     * @var array
     */
    protected $_aCategoryArticleList = null;
    ...
    {
        if ($this->_aCategoryArticleList === null) {
            $this->_aCategoryArticleList = [];
            if ($this->_getLoadActionsParam()) {
                // newest articles
                $oArtList = oxNew(\OxidEsales\Eshop\Application\Model\ArticleList::class);
                $oArtList->loadCategoryArticlesNew();
                if ($oArtList->count()) {
                    $this->_aCategoryArticleList = $oArtList;
                }
            }
        }

        return $this->_aCategoryArticleList;
    }
