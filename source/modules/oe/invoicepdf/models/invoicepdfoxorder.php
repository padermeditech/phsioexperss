...
class InvoicepdfOxOrder extends InvoicepdfOxOrder_parent
{
    ...
    protected function _setBillingAddressToPdf($oPdf)
    {
		...
		$oPdf->text(15, 71, $this->oxorder__oxbilladdinfo->getRawValue());
        $oPdf->text(15, 75, $this->oxorder__oxbillstreet->getRawValue() . ' ' . $this->oxorder__oxbillstreetnr->value);
        $oPdf->setFont($oPdfBlock->getFont(), 'B', 10);
        $oPdf->text(15, 79, $this->oxorder__oxbillzip->value . ' ' . $this->oxorder__oxbillcity->getRawValue());
        $oPdf->setFont($oPdfBlock->getFont(), '', 10);
        $oPdf->text(15, 83, $this->oxorder__oxbillcountry->getRawValue());
        $oPdf->text(15, 87, $this->oxorder__oxbillemail->getRawValue());
		$oPdf->text(15, 91, $this->oxorder__oxbillfon->value);
    }
    ...
    protected function _setDeliveryAddressToPdf($oPdf)
    {
        ...
        $oPdf->text(15, 93, $this->translate('ORDER_OVERVIEW_PDF_DELIVERYADDRESS'));
        $oPdf->setFont($oPdfBlock->getFont(), '', 10);
		$oPdf->text(15, 96, $sSal. ' ' . $this->oxorder__oxdellname->getRawValue() . ' ' . $this->oxorder__oxdelfname->getRawValue());
        $oPdf->text(15, 100, $this->oxorder__oxdelcompany->getRawValue());
		$oPdf->text(15, 104, $this->oxorder__oxdeladdinfo->getRawValue());
        $oPdf->text(15, 108, $this->oxorder__oxdelstreet->getRawValue() . ' ' . $this->oxorder__oxdelstreetnr->value);
        $oPdf->setFont($oPdfBlock->getFont(), 'B', 10);
        $oPdf->text(15, 112, $this->oxorder__oxdelzip->value . ' ' . $this->oxorder__oxdelcity->getRawValue());
        $oPdf->setFont($oPdfBlock->getFont(), '', 10);
        $oPdf->text(15, 116, $this->oxorder__oxdelcountry->getRawValue());
		$oPdf->text(15, 120, $this->oxorder__oxdelfon->value);
    }
    ...
    protected function _setOrderArticlesToPdf($oPdf, &$iStartPos, $blShowPrice = true)
    {
        ...
        foreach ($this->_oArticles as $key => $oOrderArt) {
            ...
            if ($oOrderArt->oxorderarticles__oxselvariant->value) {
				//$myArray = explode(',', $oOrderArt->oxorderarticles__oxselvariant->value);
				$myArray = preg_split('/,(\s)/', $oOrderArt->oxorderarticles__oxselvariant->value);
                $iStartPos = $iStartPos + 4;
				foreach ($myArray as $key =>$ivar) {
					if ($iStartPos > 243) {
                	$this->pdffooter($oPdf);
                	$iStartPos = $this->pdfheaderplus($oPdf);
                	$oPdf->setFont($oPdfBlock->getFont(), '', 10);
					$oPdf->text(45, $iStartPos,"-".$ivar);
					$iStartPos=$iStartPos+4;
    ...
    public function exportStandart($oPdf)
    {
		...
		if ($this->oxorder__oxremark->value) {
            $oPdf->text(15, $siteH + 20, $this->translate('ORDER_USER_REMARK') . $this->oxorder__oxremark->getRawValue());
        }
    }
    ...
    public function exportDeliveryNote($oPdf)
    {
        ...
        if ($this->oxorder__oxdelsal->value) {
			...
			$oPdf->text(15, 71, $this->oxorder__oxbilladdinfo->getRawValue());
            $oPdf->text(15, 75, $this->oxorder__oxdelstreet->getRawValue() . ' ' . $this->oxorder__oxdelstreetnr->value);
            $oPdf->setFont($oPdfBlock->getFont(), 'B', 10);
            $oPdf->text(15, 79, $this->oxorder__oxdelzip->value . ' ' . $this->oxorder__oxdelcity->getRawValue());
            $oPdf->setFont($oPdfBlock->getFont(), '', 10);
            $oPdf->text(15, 83, $this->oxorder__oxdelcountry->getRawValue());
			$oPdf->text(15, 87, $this->oxorder__oxdelfon->value);
        } else {
