...
" >
    <head>
		...
		<!-- Google tag (gtag.js) -->
		<script async src="https://www.googletagmanager.com/gtag/js?id=G-LYH53ZQQW6"></script>
		<script>
		...
		return(res)
		  ...
		  function zuAnalytics(){
			if(document.cookie){
				essenziell = getCookie('essenziell');
				if(essenziell == 'yes'){
					  window.dataLayer = window.dataLayer || [];
					  function gtag(){dataLayer.push(arguments);}
					  gtag('js', new Date());

					  gtag('config', 'G-LYH53ZQQW6');
					  console.log("Cookies akzeptiert");
				}else{
					console.log("Cookies nicht akzeptiert");
				}
			}else{
				console.log("Cookies nicht akzeptiert");
			}
...
<?php echo smarty_function_oxscript(array('include' => $this->_tpl_vars['oViewConf']->getModuleUrl('jxpysioxpresstheme','out/src/js/theme.js'),'priority' => 10), $this);?>

<?php echo smarty_function_oxscript(array('include' => $this->_tpl_vars['oViewConf']->getModuleUrl('dgsearch','out/azure/src/dgsearch.js'),'priority' => 10), $this);?>

<?php $this->assign('_dgSearchurl', $this->_tpl_vars['oViewConf']->getBaseDir()); ?>
<?php echo smarty_function_oxscript(array('add' => "$('#searchParam').dgSearchList({ searchUrl: '".($this->_tpl_vars['_dgSearchurl'])."index.php?add=1' });"), $this);?>

<?php $this->assign('oConf', $this->_tpl_vars['oViewConf']->getConfig()); ?>
<input id="dgSearchDelay" type="hidden" name="dgSearchDelay" value="<?php echo ((is_array($_tmp=@$this->_tpl_vars['oConf']->getConfigParam('dgSearchDelay'))) ? $this->_run_mod_handler('default', true, $_tmp, '1') : smarty_modifier_default($_tmp, '1')); ?>
" />
<input id="dgSearchAlpha" type="hidden" name="dgSearchAlpha" value="<?php echo ((is_array($_tmp=@$this->_tpl_vars['oConf']->getConfigParam('dgSearchAlpha'))) ? $this->_run_mod_handler('default', true, $_tmp, '1') : smarty_modifier_default($_tmp, '1')); ?>
" />
 <?php if ($this->_tpl_vars['oView']->getClassName() == 'search'): ?>

 <?php endif; ?>
		
		
		<?php echo smarty_function_oxscript(array('include' => "js/widgets/cookieconsent.js"), $this);?>
